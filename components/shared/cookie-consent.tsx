...
export function CookieConsent({
    ...
    useEffect(() => {
        try {
            ...
            // Detect subdomain and build absolute URL to root domain
            const host = window.location.host;
            const protocol = window.location.protocol;
            const subdomain = host.split('.')[0];

            // Check if we're on a subdomain (not www or the root domain)
            if (subdomain && subdomain !== 'localhost' && host.includes('.')) {
                // Extract root domain (e.g., localhost:3000 from pos.localhost:3000)
                const rootDomain = host.replace(subdomain + '.', '');
                setCookiesUrl(`${protocol}//${rootDomain}/cookies`);
            }
        } catch (error) {
                ...
                <div className="grid gap-2">
                    ...
                    <div className="p-3 sm:p-4">
                        <p className="text-xs sm:text-sm font-normal text-start text-muted-foreground">
                            ...
                            <br />
                            <a href={cookiesUrl} className="text-xs underline">
                                Learn more.
                            </a>
                        </p>
                    </div>
